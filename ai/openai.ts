...
import { ChatMessageProps } from "@/app/(main)/(routes)/papers/[paperId]/_components/chat/chat-message";

const openai = new OpenAI({
  apiKey: process.env.OPENAI_API_KEY,
});

export async function OpenAIChat(fileKey: string, prompt: string, prevMessages: ChatMessageProps[] | undefined) {
  try {
    let proxy_url = ""
    ...
    const vectors_path = `${DATA_DIR}/faiss/${fileKey}`;
