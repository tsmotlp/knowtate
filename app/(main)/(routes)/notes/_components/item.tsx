...
export const Item = ({
  ...
  const archiveNote = useMutation(api.files.archiveFile);
  ...
  const handleCreateNote = (title: string, type: Doc<"files">["type"]) => {
    if (!nodeId) return;
    const promise = createNote({ title: title, parentFile: nodeId, type: type })
  ...
  const onArchive = (
    ...
    if (!nodeId) return;
    const promise = archiveNote({ fileId: nodeId })
